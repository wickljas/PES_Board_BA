...
EncoderCounter::EncoderCounter(PinName a, PinName b) {
    ...
    if ((a == PA_6) && (b == PC_7)) {
        ...
        RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN;    // manually enable port A
        ...
        GPIOC->AFR[0] &= ~(0xF << 4*7);         // reset alternate function of port C7
        ...
        GPIOA->MODER &= ~GPIO_MODER_MODER7;     // reset port C7
        GPIOA->MODER |= GPIO_MODER_MODER7_1;    // set alternate mode of port C7
        GPIOA->PUPDR &= ~GPIO_PUPDR_PUPDR7;     // reset pull-up/pull-down on port C7
        GPIOA->PUPDR |= GPIO_PUPDR_PUPDR7_1;    // set input as pull-down
        GPIOA->AFR[0] &= ~(0xF << 4*7);         // reset alternate function of port C7
        GPIOA->AFR[0] |= 2 << 4*7;              // set alternate funtion 2 of port C7
        ...
        GPIOB->AFR[0] &= ~(0xF << 4*7);           // reset alternate function of port B7
